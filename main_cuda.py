...
def downloadFileFor5f(url: str, directoryPath: str, filename: str) -> bool:
    ...
    if os.path.isfile(fullPathWithoutExtname+'.m3u8') and os.stat(fullPathWithoutExtname+'.m3u8').st_size > 0:
        ...
        subprocess.run(['ffmpeg', '-protocol_whitelist', 'file,crypto,https,tls,tcp', '-hwaccel', 'cuvid', '-hwaccel_output_format', 'cuda', '-c:v', 'h264_cuvid', '-i', fullPathWithoutExtname+'.m3u8', '-c:v', 'h264_nvenc', fullPathWithoutExtname+'.mp4'])
    else:
